...
void Tile::setElevationData(ElevationData* ed, int level, ElevationDataProvider* edp){
  
  if (_elevationDataLevel < level){
    _elevationData = ed;
    _elevationDataLevel = level;
    
    if (_elevationDataLevel == _level){ //If elevation data is solved
      if (_subtiles != NULL) {
        const int subtilesSize = _subtiles->size();
        for (int i = 0; i < subtilesSize; i++) {
          Tile* subtile = _subtiles->at(i);
          subtile->ancestorChangedElevationData(this, edp);
        }
      }
    }
  }
...
void Tile::getElevationDataFromAncestor(ElevationDataProvider* elevationDataProvider){
  
}

void Tile::ancestorChangedElevationData(Tile* ancestor, ElevationDataProvider* edp){
  
  if (_elevationDataLevel < ancestor->getLevel()){
    edp->getSubview
