...
public class CachedDownloader
  ...
  private final IStorage _cacheStorage;
  ...
  public CachedDownloader(final IDownloader downloader,
                          final IStorage cacheStorage) {
  ...
  public final long requestBuffer(final URL url,
                                  final long priority,
                                  IBufferDownloadListener listener,
                                  final boolean deleteListener) {
    _requestsCounter++;

    final IByteBuffer cachedBuffer = _cacheStorage.isAvailable()
                                                                ? _cacheStorage.readBuffer(url)
                                                                : null;
    if (cachedBuffer == null) {
      // cache miss
      return _downloader.requestBuffer(url, priority,
          new BufferSaverDownloadListener(this, _cacheStorage, listener,
              deleteListener), true);
    }
    else {
      // cache hit
  ...
  public final long requestImage(final URL url,
                                 final long priority,
                                 IImageDownloadListener listener,
                                 final boolean deleteListener) {
    _requestsCounter++;

    final IImage cachedImage = _cacheStorage.isAvailable()
                                                          ? _cacheStorage.readImage(url)
                                                          : null;
    if (cachedImage == null) {
      // TODO: HE CAMBIADO ESTO PARA PROBAR EL PRECACHEO
      // cache miss
      // return _downloader.requestImage(url, priority,
      // new ImageSaverDownloadListener(this, _cacheStorage, listener,
      // deleteListener), true);
