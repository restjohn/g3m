...
public class G3MWidget {
  ...
  public static G3MWidget create(final FrameTasksExecutor frameTasksExecutor,
                                 ...
                                 final Planet planet,
                                 final java.util.ArrayList<ICameraConstrainer> cameraConstraint,
                                 final Renderer renderer,
                                 final Renderer busyRenderer,
                                 final EffectsScheduler scheduler,
                                 final int width,
                                 final int height,
    ...
    return new G3MWidget(frameTasksExecutor, factory, stringUtils,
        threadUtils, logger, gl, texturesHandler, textureBuilder, downloader,
        planet, cameraConstraint, renderer, busyRenderer, scheduler, width,
        height, backgroundColor, logFPS, logDownloaderStatistics);
  }
  ...
  public final void render() {
    ...
    // give to the CameraContrainers the opportunity to change the
    // nextCamera
    ...
    final RenderContext rc = new RenderContext(_frameTasksExecutor, _factory,
        _stringUtils, _threadUtils, _logger, _planet, _gl, _currentCamera,
        _nextCamera, _texturesHandler, _textureBuilder, _downloader,
        _effectsScheduler, _factory.createTimer());
    ...
    final Renderer selectedRenderer = _rendererReady ? _renderer
                                                    : _busyRenderer;
    ...
    //
    // const Vector3D intersection = _planet->closestIntersection(origin,
    // ray);
    ...
    //
    // const double distanceToTerrain =
    // clamp(intersection.sub(cameraPosition).length(),
    // minDistance,
    ...
    //
    // // Clear the scene
    // const Color dayColor = Color::fromRGBA((float) 0.5, (float) 0.5, 1,
    // 1);
    ...
    if (_logFPS) {
      ...
      if ((_renderStatisticsTimer == null)
        ...
        final double averageTimePerRender = (double) _totalRenderTime
                                            / _renderCounter;
  ...
  public final void onTouchEvent(final TouchEvent myEvent) {
    if (_rendererReady) {
      final EventContext ec = new EventContext(_factory, _stringUtils,
          _threadUtils, _logger, _planet, _downloader, _effectsScheduler);
  ...
  public final void onPause() {
    final InitializationContext ic = new InitializationContext(_factory,
        _stringUtils, _threadUtils, _logger, _planet, _downloader,
        _effectsScheduler);
  ...
  private G3MWidget(final FrameTasksExecutor frameTasksExecutor,
                    final IFactory factory,
                    final IStringUtils stringUtils,
                    final IThreadUtils threadUtils,
                    final ILogger logger,
                    ...
                    final Renderer renderer,
                    final Renderer busyRenderer,
                    final EffectsScheduler effectsScheduler,
                    final int width,
                    final int height,
                    final Color backgroundColor,
                    final boolean logFPS,
