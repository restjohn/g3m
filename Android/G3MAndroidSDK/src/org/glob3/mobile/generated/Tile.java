...
public class Tile {
  ...
  private Mesh getTessellatorMesh(final RenderContext rc,
  ...
  private Mesh getDebugMesh(final RenderContext rc,
                            final TileRenderContext trc) {
    if (_debugMesh == null) {
  ...
  private boolean isVisible(final RenderContext rc,
                            final TileRenderContext trc) {
    ...
    // return extent->touches(
    // rc->getCurrentCamera()->getHalfFrustuminModelCoordinates() );
  }


  private boolean meetsRenderCriteria(final RenderContext rc,
                                      final TileRenderContext trc) {
    ...
    final TileTexturizer texturizer = trc.getTexturizer();
    if (texturizer != null) {
    ...
    // if (projectedSize <= (parameters->_tileTextureWidth *
    // parameters->_tileTextureHeight * 2)) {
    // return true;
    // }
    ...
    final int __TODO_tune_render_budget;
  ...
  private java.util.ArrayList<Tile> createSubTiles() {
    ...
    final java.util.ArrayList<Tile> subTiles = new java.util.ArrayList<Tile>();
    subTiles.add(createSubTile(lower.latitude(), lower.longitude(), midLat,
        midLon, nextLevel, 2 * _row, 2 * _column));

    subTiles.add(createSubTile(lower.latitude(), midLon, midLat,
        upper.longitude(), nextLevel, 2 * _row, 2 * _column + 1));

    subTiles.add(createSubTile(midLat, lower.longitude(), upper.latitude(),
        midLon, nextLevel, 2 * _row + 1, 2 * _column));

    subTiles.add(createSubTile(midLat, midLon, upper.latitude(),
        upper.longitude(), nextLevel, 2 * _row + 1, 2 * _column + 1));

    return subTiles;
  }


  private void rawRender(final RenderContext rc,
                         final TileRenderContext trc) {

    final Mesh tessellatorMesh = getTessellatorMesh(rc, trc);
    ...
    if (tessellatorMesh != null) {
      if (texturizer == null) {
        ...
        final boolean needsToCallTexturizer = (_texturizedMesh == null)
                                              || isTexturizerDirty();

        if (needsToCallTexturizer) {
          final int __TODO_tune_render_budget;

          _texturizedMesh = texturizer.texturize(rc, trc, this,
              tessellatorMesh, _texturizedMesh);
        }
  ...
  private void debugRender(final RenderContext rc,
                           final TileRenderContext trc) {
    final Mesh debugMesh = getDebugMesh(rc, trc);
    if (debugMesh != null) {
      debugMesh.render(rc);
    }
  }
  ...
  private Tile createSubTile(final Angle lowerLat,
                             ...
                             final int row,
                             final int column) {
    return new Tile(_texturizer, this, new Sector(new Geodetic2D(lowerLat,
        lowerLon), new Geodetic2D(upperLat, upperLon)), level, row, column);
  }


  public java.util.ArrayList<Tile> getSubTiles() {
    if (_subtiles == null) {
  ...
  private void prune(final TileRenderContext trc) {
    if (_subtiles != null) {

      // printf("= pruned tile %s\n", getKey().description().c_str());

      final TileTexturizer texturizer = (trc == null) ? null
                                                     : trc.getTexturizer();
  ...
  // C++ TO JAVA CONVERTER TODO TASK: The implementation of the following
  // method could not be found:
  // Tile(Tile that);

  private void ancestorTexturedSolvedChanged(final Tile ancestor,
  ...
  private void deleteTexturizedMesh(final TileRenderContext trc) {
  ...
  public Tile(final TileTexturizer texturizer,
              final Tile parent,
              final Sector sector,
              final int level,
              final int row,
  ...
  public final void render(final RenderContext rc,
                           final TileRenderContext trc,
                           final java.util.LinkedList<Tile> toVisitInNextIteration) {
    final TilesStatistics statistics = trc.getStatistics();
    ...
    if (isVisible(rc, trc)) {
      ...
      if ((toVisitInNextIteration == null) || meetsRenderCriteria(rc, trc)) {
        ...
        final java.util.ArrayList<Tile> subTiles = getSubTiles();
        ...
        for (int i = 0; i < subTilesSize; i++) {
          final Tile subTile = subTiles.get(i);
  ...
  public final void setTextureSolved(final boolean textureSolved) {
  ...
  public final void setTexturizerDirty(final boolean texturizerDirty) {
  ...
  public final void setTexturizerData(final ITexturizerData texturizerData) {
  ...
  // C++ TO JAVA CONVERTER WARNING: 'const' methods are not available in
  // Java:
  // ORIGINAL LINE: const Tile* getDeepestTileContaining(const Geodetic3D&
  // position) const
  public final Tile getDeepestTileContaining(final Geodetic3D position) {
