...
public class G3MWidget_Android
  ...
  // private final LinkedList<Runnable> _pausedRunnableQueue = new
  // LinkedList<Runnable>();
  ...
  private void initWidget() {
    ...
    final TilesRenderParameters parameters = TilesRenderParameters.createDefault(
        renderDebug, useTilesSplitBudget, forceTopLevelTilesRenderOnStart);
  ...
  private void initWidget(final CameraRenderer cameraRenderer,
    ...
    _downloader = new CachedDownloader(new Downloader_Android(8,
        connectTimeout, readTimeout), _storage);
    ...
    if ((_layerSet != null) && (_layerSet.size() > 0)) {
      ...
      if (true) {
        ...
        texturizer = new SingleImageTileTexturizer(parameters,
            singleWorldImage, false);
      }


      final boolean showStatistics = false;

      final TileRenderer tr = new TileRenderer(new EllipsoidalTileTessellator(
          parameters._tileResolution, true), texturizer, parameters,
          showStatistics);
    ...
    _g3mWidget = G3MWidget.create(frameTasksExecutor, factory, stringUtils,
        threadUtils, stringBuilder, math, jsonParser, logger, gl,
        texturesHandler, textureBuilder, _downloader, planet,
        _cameraConstraints, composite, busyRenderer, scheduler, width, height,
